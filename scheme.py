...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"  
        checker=scheme_eval(expr.first,env)
        if isinstance(checker, MacroProcedure):
            code=expr.second.map(lambda x:x)
            return scheme_eval(checker.apply_macro(expr.second, env), env)
        code=expr.second.map(lambda x: scheme_eval(x, env))
        check_procedure(checker)
        return scheme_apply(checker,code,env)
        

        # one=expr.first 
        # two=expr.second
    
        # keep=scheme_eval(one,env)
        # check_procedure(keep)
        
        # args=two.map(lambda x: scheme_eval(x,env))             
        # return scheme_apply(keep,args,env)
...
def eval_all(expressions, env):
    ...
    if expressions==nil:
        ...
        def helper (expressions,env,collect):
            ...
            #    return collect[-1] 
                ...
                one=expressions.first
                rest=expressions.second
                collect.append(scheme_eval(one,env))

                return helper(rest, env,collect)
            
            #Pair(scheme_eval(one,env), Pair())
            #return 
        return helper (expressions,env,[])
...
class Frame(object):
    ...
    def define(self, symbol, value):
        # BEGIN PROBLEM 3
        "*** YOUR CODE HERE ***"
        #if symbol not in self.bindings: #=self.value
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            ...
            for i in range (size):
                ...
                formals=formals.second#.first
                vals=vals.second#.first
            return child



            #    child.define(formals[i],vals[i])
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        target2=scheme_eval(expressions.second.first,env)

        env.define(target,target2)
        return target
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        new=LambdaProcedure(target.second, expressions.second,env)

        env.define(target.first,new)
        return target.first


        # END PROBLEM 10eval_all
    else:
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    #LambdaProcedure.def __init__(self, formals, body, env):
    #return LambdaProcedure.__init__(expressions,formals, expressions.second, env)
    lamb=LambdaProcedure(formals, expressions.second,env)
    return lamb
    #return expressions
...
def do_and_form(expressions, env):
    ...
    if expressions==nil:
        ...
        return scheme_eval(expressions.first,env,True)      #changed today      
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        #check_formals
            #sym.append
        sym=Pair(bindings.first.first, sym)
        val=Pair(scheme_eval(bindings.first.second.first,env),val)
        #else:
        #    raise SchemeError
        bindings=bindings.second
    check_formals(sym)
    return env.make_child_frame(sym,val)



            #add both to the pairs, but evaluate the expression 
    '''
    def make_child_frame(self, formals, vals):
        """Return a new local frame whose parent is SELF, in which the symbols
        in a Scheme list of formal parameters FORMALS are bound to the Scheme
    ...
    #return env.make_child_frame(bindings.first,scheme_eval(bindings.second,env))
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    check_form(expressions,2)
    one=expressions.first
    #two=expressions.second

    if isinstance(one,Pair) and scheme_symbolp(one.first):
        env.bindings[one.first]=MacroProcedure(one.second,expressions.second,env)
        #env.define(one.first,proc) 
        return one.first
    else:
        raise SchemeError
    # END Problem 21
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 16
    "*** YOUR CODE HERE ***"
    muu=MuProcedure(formals,expressions.second)
    return muu
    # END PROBLEM 16
