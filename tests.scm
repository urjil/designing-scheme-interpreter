...
(define pythagorean-c (+ (* a-py a-py) (* b-py b-py)))
pythagorean-c
; expect 25
...
(define x 15)
(define y 2)
(define func1 (lambda (x y) (+ (* y y) (+ x x))))
(func1 x y)
; expect 34
...
(define (cut-first-and-double x)
  (let ((x (cdr x)))
        (append x x)))
(cut-first-and-double '(1 2 3 4))
; expect (2 3 4 2 3 4)

(eq? 2 2.00)
; expect #t

(define (over-or-less-15 x)
  (cond ((> x 15) 'over)
        ((< x 15) 'less)
        (else 'equal)))
(over-or-less-15 15)
